...
<table id="example-1" class="table table-striped dt-responsive display" cellspacing="0" width="100%">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
            <a href="{{ route('foundation-image.show', $foundation_category_row->id) }}"  rel="tooltip" data-color-class="danger" data-animate=" animated fadeIn " data-toggle="tooltip" data-original-title="View Foundation Image ({{ $foundation_category_row->name }})" class="btn btn-warning btn-sm" >
                    <span class="badge badge-primary">
                        {{ $foundation_category_row->foundation_images_count }}
                    </span>
                     View Foundation Images/Sort Order
                </a>
            </td>
            <td>
                <div class="d-flex gap-2" style="display: flex;">
                    <a href="javascript:void(0);" data-editfoundationcategory-popup="true" data-fcid="{{ $foundation_category_row->id }}" data-size="lg" data-title="Edit {{ $foundation_category_row->name }}" data-url="{{ route('foundation-category.edit', $foundation_category_row->id) }}" data-bs-toggle="tooltip" data-bs-original-title="Edit {{ $foundation_category_row->name }}" class="btn btn-primary btn-sm">
                        <i class="fa fa-pencil icon-xs"></i>
                    </a>
                    
                    <form method="POST" action="{{ route('foundation-category.destroy', $foundation_category_row->id) }}" style="margin-left: 10px;">
                        @csrf
